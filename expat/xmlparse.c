...
#define MALLOC(s) (parser->m_mem.malloc_fcn(parser->m_mem.priv,(s)))
#define REALLOC(p,s) (parser->m_mem.realloc_fcn(parser->m_mem.priv,(p),(s)))
#define FREE(p) (parser->m_mem.free_fcn(parser->m_mem.priv,(p)))
             ...
             DTD *dtd)
{
  ...
  if (memsuite) {
    ...
    parser = (XML_Parser)
      memsuite->malloc_fcn(memsuite->priv,sizeof(struct XML_ParserStruct));
    if (parser != NULL) {
      mtemp = (XML_Memory_Handling_Suite *)&(parser->m_mem);
      mtemp->malloc_fcn = memsuite->malloc_fcn;
      mtemp->realloc_fcn = memsuite->realloc_fcn;
      mtemp->free_fcn = memsuite->free_fcn;
      mtemp->priv = memsuite->priv;
    }
  }
#if 0
  else {
