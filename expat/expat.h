...
typedef struct {
  void *(*malloc_fcn)(void *priv,size_t size);
  void *(*realloc_fcn)(void *priv,void *ptr, size_t size);
  void (*free_fcn)(void *priv,void *ptr);
  void *priv;
...
/* local additions
                - Mike */
/* (re)Parses character data (handles char refs and predefined entities)*/
XMLPARSEAPI(int)
XML_ParseCharacterData(XML_Parser parser,char **source,int len,
	wchar_t *dest,int destlen);

/* see if we are expanding any entities now */
XMLPARSEAPI(int)
XML_IsExpanding(XML_Parser parser);

/* simpy convert encoding for cdata sections */
XMLPARSEAPI(int)
XML_ConvertCharacterData(XML_Parser parser,char **source,int len,
	 wchar_t *dest,int destlen);
